...
import com.github.xpenatan.gdx.backends.teavm.*;
...
public class TeaApplication implements Application, Runnable {
    ...
    private final TeaWindow window;
    ...
    private final ObjectMap<String, Preferences> prefs = new ObjectMap<>();

    private TeaClipboard clipboard;

    private final Array<Runnable> runnables = new Array<Runnable>();
    private final Array<Runnable> runnablesHelper = new Array<Runnable>();
    ...
    private void init() {
        ...
        var assetDownload = new dev.ultreon.quantum.teavm.AssetDownloadImpl(config.showDownloadLogs);
        AssetInstance.setInstance(assetDownload);

        TeaWindow currentWindow = TeaWindow.get();
        if (currentWindow.getLocation().getHref().startsWith("https://playqv.ultreon.dev/")) {
            hostPageBaseURL = "https://playqv.ultreon.dev/";
        } else if (currentWindow.getLocation().getHref().startsWith("http://localhost:")
                   || currentWindow.getLocation().getHref().startsWith("http://127.0.0.1:")
                   || currentWindow.getLocation().getHref().startsWith("http://0.0.0.0:")
                   || currentWindow.getLocation().getHref().startsWith("https://localhost:")
                   || currentWindow.getLocation().getHref().startsWith("https://127.0.0.1:")
                   || currentWindow.getLocation().getHref().startsWith("https://0.0.0.0:")
        ) {
            hostPageBaseURL = currentWindow.getLocation().getHref();
        } else if (currentWindow.getLocation().getHref().matches("https?://[0-9]+.discordsays.com(/.*)?")) {
            hostPageBaseURL = currentWindow.getLocation().getHref().replaceAll("^(https?://[0-9]+.discordsays.com)(/.*)?$", "$1/.proxy/");
        } else {
            TeaVMConsole.error("Unsupported URL: " + currentWindow.getLocation().getHref());
            throw new GdxRuntimeException("Unsupported URL: " + currentWindow.getLocation().getHref());
        }
